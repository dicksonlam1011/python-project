...
relese_schedule = pd.ExcelFile("{}/Deployment_release_schedule_Backup.xlsx".format(project_source_path))
...
def getFunctionDataToImage(BW,Urg,SV):
    ...
    # print(dict)
    if not os.path.isdir("{}/{}".format(project_path,BW_releaseSch)):
        os.mkdir("{}/{}".format(project_path, BW_releaseSch))

    with open(os.path.join("{}/{}".format(project_path, BW_releaseSch),"FunctionDataToImage-{}.log".format(BW_releaseSch)), mode="w",encoding="utf8") as file: # Open a file
...
def getPpmStatistics(BW, Urg, SV):
    ...
    with open(os.path.join("{}/{}".format(project_path, BW_releaseSch),"result-{}.log".format(BW_releaseSch)), mode="w",encoding="utf8") as file: # Open a file
...
def writeBackup(BW,Urg,SV):
    ...
    with open(os.path.join("{}/{}".format(project_path, BW_releaseSch),"data-{}.log".format(BW_releaseSch)), mode="w",encoding="utf8") as file: # Open a file
        BW_data=BW.to_string()
        Urg_data=Urg.to_string()
        SV_data=SV.to_string()
        file.write(BW_data)
...
def run_main_program():
    ...
    getPpmStatistics(BW,Urg,SV)
    writeBackup(BW,Urg,SV)
    getFunctionDataToImage(BW,Urg,SV)
